...
- extensions を拡張していくやり方だとスキーマが定義出来ないのでフロントとサーバ両方で型定義が必要
  - 口約束レベルなのでミスの可能性がある
  - フロント側で型アサーションをする必要がある

## ユーザに見せていいエラーかどうかレスポンスだけをみて判断できるようにしたい

- BE からのエラーメッセージを画面に表示させる実装をするので
- この形のエラーメッセージはユーザに見せて良い、この形のエラーは見せず「予期しないエラーが発生しました」程度の表示にしておく、をきっちり判断できるようにする

# 最終的な判断

以上の基本方針を踏まえ、私のプロダクトでは下記のようにエラーを扱っていく判断をしました。

<!-- ブログシステムを例に交えつつ説明を入れています。 -->

## 判断内容

<!-- TODO: 文章で書く -->

- Result 型を union 型で定義する感じにした
- graphql-codegen の恩恵を受けられる
- フィールドでの定義だと nullable フィールドが大量発生する

## エラーの分類

今回検討しているのはサーバのエラーメッセージをクライアントに適切に伝えることでした。しかし一括りに「エラー」といってもすべてに型付けをしていくのは難しいです。そこでエラーを「ユーザエラー」「例外エラー」の 2 通りで区別するようにしました。

### ユーザエラー

クライアントに詳細を伝えるべきエラーをここに分類しています。エラーの内容によってクライアントが処理を分岐してくれることを期待する場合は、そのエラーを「ユーザエラー」として扱う、という基準を設けました。基本的にはバリデーションエラー等のクライアント起因のエラーが多く分類されますが、サーバ起因のエラーでも「失敗しました」以上のフィードバックが必要なエラーはユーザエラーとします。(一部の処理に失敗した場合など。) 具体的には、下記のようなエラーがユーザエラーに分類されます。

- 入力値のバリデーションエラー
- ユーザの保存処理は成功したが、招待メールの送信に失敗した
- システム X とシステム Y からデータを取得しなければいけないところ、システム X からしか取得できなかった

また、前述した GraphQL スキーマとして提供するエラーはすべてユーザエラーに分類されます。以上のエラーを Result 型で定義した場合の例を示します。

```graphql
"入力値のバリデーションエラー"
type InviteUserErrorValidation {
  message: String!
  details: [ErrorDetail!]! # エラーが起きたフィールド名とかが入る想定
}

"ユーザの保存処理は成功したが、招待メールの送信に失敗した"
type InviteUserErrorSendEmail {
  message: String!
  user: User! # ユーザの保存処理自体は完了しているので作成済みのリソースを返す
}

"システム X とシステム Y からデータを取得しなければいけないところ、システム X からしか取得できなかった"
type ErrorGetY {
  message: String!
  xData: XData! # システム X からはデータが取得できているので返す
}
```
